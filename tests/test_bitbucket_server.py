...
class TestBitbucketServer(TestCase):
    ...
    def test_get_users(self):
        ...
        for idx, resulting_user in enumerate(result_users):
            ...
            self.assertEqual(resulting_user.get('login'), input_user.get('name'), "user logins should match")
            self.assertEqual(resulting_user.get('name'), input_user.get('displayName'), "user names should match")
            self.assertEqual(resulting_user.get('email'), input_user.get('emailAddress'), "user emails should match")
    ...
    def test_get_projects(self):
        ...
        self.assertEqual(result_project['login'], input_project['key'], "resulting project login does not match input")
        self.assertEqual(result_project['name'], input_project['name'], "resulting project name does not match input")
        self.assertEqual(result_project['url'], input_project['links']['self'][0]['href'], "resulting project url does not match input")
    ...
    def test_get_repos(self):
        ...
        self.assertEqual(result_repo['id'], input_repo['id'], "resulting repo id does not match input")
        ...
        for idx, test_branch in enumerate(test_branches):
            ...
            self.assertEqual(result_branch['name'], test_branch['displayId'], "resulting branch name does not match input")
            self.assertEqual(result_branch['sha'], test_branch['latestCommit'], "resulting branch sha does not match input")
    ...
    def test_get_branch_commits(self):
        ...
        for idx, test_commit in enumerate(test_commits):
            ...
            self.assertEqual(result_commit['author'], test_commit['author'], "resulting author does not match input")
            expected_url = test_repos[0]['links']['self'][0]['href'].replace('browse', f'commits/{test_commit["id"]}')
            self.assertEqual(result_commit['url'], expected_url, "resulting url does not match input")
            self.assertEqual(result_commit['message'], test_commit['message'], "resulting message does not match input")
    ...
    def test_get_pull_requests(self):
        ...
        self.assertEqual(result_pr['author']['name'], input_pr['author']['user']['displayName'], "resulting PR author name does not match input")
        self.assertEqual(result_pr['title'], input_pr['title'], "resulting PR title does not match input")
        self.assertEqual(result_pr['body'], input_pr['description'], "resulting PR body does not match input")
        self.assertTrue(result_pr['is_closed'])
        self.assertFalse(result_pr['is_merged'])
        self.assertEqual(result_pr['url'], input_pr['links']['self'][0]['href'], "resulting PR url does not match input")
        self.assertEqual(result_pr['base_repo']['name'], input_pr['toRef']['repository']['name'], "resulting PR base repo name does not match input")
        self.assertEqual(result_pr['base_branch'], input_pr['toRef']['displayId'], "resulting PR base branch does not match input")
        self.assertEqual(result_pr['head_repo']['name'], input_pr['fromRef']['repository']['name'], "resulting PR repo name does not match input")
