...
class TestBitbucketCloudClient(TestCase):
    ...
    def test_download_with_429_timeout(self):
        with requests_mock.Mocker() as m:
            ...
            request_time = datetime.now()
            results = self.bitbucket_connection.get_raw_result(URI)  # hit 429, wait, get results delayed
            return_time = datetime.now()
            self.assertGreaterEqual((return_time - request_time).total_seconds(), self.faux_ratelimit_wait_time)
            json_response = json.loads(results.text)
            self.assertGreaterEqual(len(json_response[0]), 19)  # number of elements in test repo json (2023-05-26)
