...
def get_all_repos(client, projects):
    ...
    for project in projects:
        ...
        for repo in api_project.repos.list():
            ...
            try:
                default_branch_name = (
                    api_repo.default_branch["displayId"]
            ...
            branches = list(
                {"name": b["displayId"], "sha": b["latestCommit"]}
...
def get_default_branch_commits(client, repos):
    for repo in repos:
        ...
        try:
            ...
            print(
                f'WARN: Got NotFoundException for branch {repo["default_branch_name"]}: {e}'
            )
            return []
    ...
    for repo in repos:
        ...
        for pr in api_repo.pull_requests.all(state="ALL", order="NEWEST"):
            ...
            yield {
                ...
                "updated_at": datetime_from_bitbucket_server_timestamp(
                    pr["updatedDate"]
                ),
